...
class MessageStateSink extends MessageSink {
  constructor (apiKey=null, userId=null) {
     ...
     * @typedef {Object} MessageStateSink~State
     ...
     * service.
     * @property {Null|String} create.messageId - The message id returned by the
     * response from the Chatbase service after a successful attempt to create
     * the message with the Chatbase service. This value is necessary to update
     * the created message on the Chatbase service.
  ...
  setAsCreateCompleted (responseBody) {
    ...
    if (responseBody && responseBody.message_id) {
      super.setMessageId(responseBody.message_id);
    }
    return this;
  }
  /**
  ...
  /**
   * Returns the message id extracted from the message create response
   * @function messageId
   * @public
   * @returns {Null|String} - returns the message-id if present
   */
  messageId () {
   ...
   */
  setIntent (intent) {
    if (this.createEntryCompleted() && !this.cannotBeUpdated()) {
      this.flagIntentForUpdate();
    }
